...
from lip_lang import (
    Program, Assign, Pipeline, PipeCap, FuncCallRHS, MethodCallRHS,
    MethodAttrRHS, TableRHS, BlockRHS, NameRHS, Atom, IdxAcc, DotAcc,
    IntLit, FloatLit, BoolLit, StringLit, PendingLit, TypeNameAtom,
    NameAtom, TableLit, BlockLit, DirectCall
)
    ...
    def get(self, key: Any) -> Any:
        if isinstance(key, bool):
            ...
            if 0 <= idx < len(self.seq):
                return self.seq[idx]
            return make_pending("get", f"index {idx} out of range")
...
PENDING_SAFE_FUNS = {'recover', 'is', 'on_pending', 'not'}
...
class Interpreter:
    ...
    def _register_types(self):
        for t in ['Int', 'Float', 'Bool', 'String', 'Block', 'Table']:
    ...
    def _get_attr(self, val: Any, name: str) -> Any:
        ...
        return make_pending("attr", f"cannot get attribute '{name}' from {type(val).__name__}")
    ...
    def _apply_pipe(self, val: Any, step: Any, env: Env) -> Any:
        if isinstance(step, FuncCallRHS):
            ...
            idx = self._to_dispatch_index(val)
            selected = table.get(idx)
    ...
    def _to_dispatch_index(self, val: Any) -> Any:
    ...
    def _call_value(self, fn: Any, upstream: Any, pos_args: list, kw_args: dict, env: Env) -> Any:
        if isinstance(fn, LipBlock):
            all_args = [upstream] + pos_args if not is_pending(upstream) or upstream is not PENDING else pos_args
            if upstream is PENDING and pos_args:
                all_args = pos_args
            elif upstream is not PENDING:
    ...
    def _call_method(self, obj: Any, name: str, pos_args: list, kw_args: dict, env: Env) -> Any:
    ...
    def _b_or(self, up, args, kw, env):
        ...
        b = args[0]
        up_true = up is True or (isinstance(up, (int, float)) and up != 0 and not isinstance(up, bool))
        b_true = b is True or (isinstance(b, (int, float)) and b != 0 and not isinstance(b, bool))
    ...
    def _b_join(self, up, args, kw, env):
        ...
        sep = args[0] if args else ""
        try:
            return sep.join(str(x) for x in up.seq)
        except:
    ...
    def _b_reduce(self, up, args, kw, env):
        ...
        for item in up.seq:
            acc = self._call_block(block, [acc, item], {}, env)
        return acc
    ...
    def _b_range(self, up, args, kw, env):
        ...
        try:
            ...
            if not args:
                start, end = 0, start
            return LipTable(list(range(start, end)))
        except:
    ...
    def _b_take(self, up, args, kw, env):
        ...
        return LipTable(up.seq[:n], up.named.copy())
    ...
    def _m_get(self, obj, args, kw, env):
        ...
        return obj.get(args[0]) if isinstance(obj, LipTable) else make_pending("get", "not a table")

    def _m_set(self, obj, args, kw, env):
        if not isinstance(obj, LipTable):
    ...
    def _m_merge(self, obj, args, kw, env):
        ...
        if not args or not isinstance(args[0], LipTable):
            return make_pending("merge", "missing table argument")
        return obj.merge(args[0])
